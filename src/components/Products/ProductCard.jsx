...
export default function ProductCard({ product }) {
  ...
  const fallbackImage = "https://images.unsplash.com/photo-1471193945509-9ad0617afabf?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=300&q=80";
    ...
    <div className="card overflow-hidden hover:scale-[1.02] transition-transform duration-300">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center text-gray-500 text-sm mb-2">
          ...
          <span>{product?.sellerInfo?.district || "District"}, {product?.sellerInfo?.region || "Region"}</span>
        </div>
        
        <div className="flex items-center text-gray-500 text-sm mb-3">
          <FaCalendarAlt className="mr-1" />
          <span>Harvested: {product?.harvestDate ? format(new Date(product.harvestDate), 'dd MMM yyyy') : "N/A"}</span>
        </div>
        ...
        <div className="flex justify-between items-center">
          <div className="text-sm">
            ...
            <span className={`font-medium ${
              ...
              {product?.availableStock ? `${product.availableStock} ${product.unit}` : "Out of stock"}
            </span>
          </div>
          <Link 
